...
- Google Cloud credential renewal from the console: the `gcloud` login behind Vertex AI expires roughly daily, and any user or admin can test the credentials and re-authenticate from a browser instead of needing a shell on the proxy host (opt-in, `general_settings.ALLOW_GCP_ADC_TOKEN_REFRESH`)
...
Google expires these credentials roughly daily. Setting `ALLOW_GCP_ADC_TOKEN_REFRESH: true`
under `general_settings` adds a **Google Cloud auth** page to the console where any user can
test them and run this login again from a browser, so renewing them does not need shell
access to the proxy host. Left off, the page and its sidebar entry do not exist.
