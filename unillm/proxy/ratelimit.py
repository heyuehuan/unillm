...
# Each press of "Run health test" is a real Gemini call, and each "Re-authenticate"
# spawns a gcloud process. Neither is expensive, but both are unauthenticated-adjacent
# in the sense that any signed-in user can trigger them, so they are capped per user
# rather than left to be pressed in a loop.
adk_action_limiter = _limiter_from_env("UNILLM_ADK_ACTION_LIMIT", (10, 60.0))
...
def reload_from_env() -> None:
    ...
    global login_audit_limiter, profile_email_limiter, adk_action_limiter
    login_attempt_limiter = _limiter_from_env("UNILLM_LOGIN_RATE_LIMIT", (30, 60.0))
    login_ip_limiter = _limiter_from_env("UNILLM_LOGIN_IP_RATE_LIMIT", (100, 60.0))
    login_user_limiter = _limiter_from_env("UNILLM_LOGIN_FAILURE_LIMIT", (5, 900.0))
    docs_limiter = _limiter_from_env("UNILLM_DOCS_RATE_LIMIT", (30, 60.0))
